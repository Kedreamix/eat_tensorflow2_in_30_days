...
- [3-1 低阶API示范](#3-1-低阶api示范)
    - [一，线性回归模型](#一线性回归模型)
    - [二，DNN二分类模型](#二dnn二分类模型)
...
![](./data/jpg/3-1-01-回归数据可视化.png)

接着我们利用tensorflow来构建数据管道迭代器，并且之后样本的读取数据是随机的
...
之后则是定义我们的模型，定义一个线性回归模型，根据y=wx+b，同时初始化我们的权重w和偏置b
...
**3，训练模型**
之后我们就可以开始训练模型，我们可以利用动态图进行调试，首先得到我们的损失，然后再通过反向传播求解我们梯度，再利用梯度下降法更新参数
...
此处范例我们利用tf.Module来组织模型变量，关于tf.Module的较详细介绍参考4.6 Autograph和tf.Module。
...
最后我们可以可视化我们的预测值和实际值的区别，我们可以看到结果还是很近人意的
