## 1-4 时间序列数据建模流程范例 (国内新冠疫情结束时间预测问题)

<!-- TOC -->

- [1-4 时间序列数据建模流程范例 (国内新冠疫情结束时间预测问题)](#1-4-时间序列数据建模流程范例-国内新冠疫情结束时间预测问题)
    - [一，准备数据](#一准备数据)
    - [二，定义模型](#二定义模型)
    - [三，训练模型](#三训练模型)
    - [四，评估模型](#四评估模型)
...
<!-- /TOC -->
国内的新冠肺炎疫情从发现至今已经持续很久了，这场起源于吃野味的灾难给大家的生活造成了诸多方面的影响。
...
本文的数据集取自tushare，数据集在本项目的 data目录下。(这里面只取了2020的三个月的数据)

![](./data/jpg/1-4-新增人数.png)
